  ...
  .then(data => {
      if (data.message === "Registro exitoso") {
        ...
        if (buttonClicked === 'saveAndConsult') {
          ...
          showModal('./tablaConsulta.html'); // Mostramos la ventana emergente y configuramos la redirección
        } else if (buttonClicked === 'saveAndExit') {
          showModal('./proyecGeneral.html'); // Mostramos la ventana emergente y configuramos la redirección
        }
      } else {
          alert(data.message);
      }
